module Api
  module V1
    module Admin
      ...
      #
      # The admin console opened on a list of companies, which answers "who
      ...
      class MetricsController < BaseController
        ...
        def companies_json
          ...
          # reports it (AdminCompanySerializer#access_open).
        ...
        def money_json
          ...
          arrears = Subscription.includes(company: :owner).sum { |s| s.arrears_cents }
        ...
        def recent_companies_json
          Company.includes(:owner, :subscription).order(created_at: :desc).limit(5).map do |company|
            {
              id: company.id,
              name: company.name,
              city: company.city,
