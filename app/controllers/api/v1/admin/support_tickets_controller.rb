module Api
  module V1
    module Admin
      # Cross-company support inbox — every problem report an owner filed
      ...
      class SupportTicketsController < BaseController
        ...
        # GET /api/v1/admin/support_tickets?status=open
        def index
          tickets = SupportTicket.includes(:company, :created_by).recent
          tickets = tickets.where(status: params[:status]) if params[:status].present?

          render json: {
            support_tickets: paginate(tickets).map { |t| AdminSupportTicketSerializer.new(t).as_json },
        ...
        def resolve
          ...
          render json: { support_ticket: AdminSupportTicketSerializer.new(@ticket).as_json }
        end

        # GET /api/v1/admin/support_tickets/:id/attachments/:attachment_id
        def attachment
